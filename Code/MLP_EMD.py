...
train_X = data_xx[:sep]   #flatten
train_y = data_y[:sep]
validate_X = data_xx[sep:]
...
model.compile(optimizer=Adagrad(lr=0.01), loss= "mae", metrics=['mae'])
model.summary()
...
history = model.fit(train_X, train_y, epochs=epoch, batch_size=batch_size, verbose=1, shuffle=False,
                    callbacks = callbacks,
...
lstm_model.compile(loss="mae",optimizer=Adagrad(lr=0.01),metrics=['mae']) #optimizer=Adagrad(lr=0.01)
