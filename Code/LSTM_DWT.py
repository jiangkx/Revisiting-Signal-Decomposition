...
dwt = np.array(pd.read_csv("../demo_data/Hs_DWT(Leak).csv"))[:,1:]   # Leak version
# dwt = np.array(pd.read_csv("../demo_data/Hs_DWT(NoLeak).csv"))[:,1:]  # No Leak version

data = np.concatenate((data,dwt),axis=1)
