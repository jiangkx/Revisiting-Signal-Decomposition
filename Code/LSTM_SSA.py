...
ssa = np.array(pd.read_csv("../demo_data/Hs_SSA(Leak).csv"))[:,1:]   # Leak version
# ssa = np.array(pd.read_csv("../demo_data/Hs_SSA(NoLeak).csv"))[:,1:]  # No Leak version

data = np.concatenate((data,ssa),axis=1)
